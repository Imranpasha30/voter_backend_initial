...
class Part(Base):
    ...
    area_id = Column(Integer, ForeignKey("areas.area_id", ondelete="RESTRICT"), nullable=True)  # ✅ NEW - Nullable for migration
    part_name_en = Column(String(255))
    part_name_v1 = Column(String(255))
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    
    # Relationships
    area = relationship("Area", back_populates="parts")  # ✅ NEW
